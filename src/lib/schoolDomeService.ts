    ...
    for (const uId of lastStandingIds) {
      try {
        ...
        if (uSnap.exists()) {
          ...
          // Credit winner's wallet directly with GP
          ...
          try {
            const txDoc = doc(collection(db, 'wallet_transactions'));
            await setDoc(txDoc, {
              id: txDoc.id,
              userId: uId,
              type: 'credit',
              category: 'school_dome_prize',
              amount: prizePerWinner,
              currency: 'GP',
              description: `School Dome Season #${seasonData.seasonNumber} Champion Prize (Equal Share)`,
              status: 'completed',
              timestamp: Date.now(),
          ...
          winners.push({
            ...
            institution: 'Nigerian Higher Institution',
            prizeWon: prizePerWinner,
          });
        }
      } catch (e) {
